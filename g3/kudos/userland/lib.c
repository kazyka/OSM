...
usr_sem_t* syscall_usr_sem_open(const char* name, int value) {
  return (usr_sem_t*) _syscall(SYSCALL_USR_SEM_OPEN,
                               (uintptr_t) name,
                               (uintptr_t) value,
                               0);
}

int syscall_usr_sem_destroy(usr_sem_t* handle) {
  return (int) _syscall(SYSCALL_USR_SEM_DESTROY,
                        (uintptr_t) handle,
                        0, 0);
}

int syscall_usr_sem_procure(usr_sem_t* handle) {
  return (int) _syscall(SYSCALL_USR_SEM_PROCURE,
                        (uintptr_t) handle,
                        0, 0);
}

int syscall_usr_sem_vacate(usr_sem_t* handle) {
  return (int) _syscall(SYSCALL_USR_SEM_VACATE,
                        (uintptr_t) handle,
                        0, 0);
}
